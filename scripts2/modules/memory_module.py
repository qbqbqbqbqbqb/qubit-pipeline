...
class MemoryModule(BaseModule):
    def __init__(self, base_path: str = ".", memory_enabled = True,  response_generator=None):
        ...
        self.logger.info(f"ChromaDB initialized with {self.collection.count()} reflection memories")
...
"""
        ...
        self.logger.info(f"MemoryManager initialized with {len(self.memories)} memories")
    ...
    def store_memory(self, content: str, memory_type: str = "semantic",
        ...
        self.logger.debug(f"Stored {memory_type} memory: {memory.id}")
        return memory.id
    ...
    async def _perform_reflection(self) -> None:
        ...
        self.logger.info(f"[Reflection] Retrieved {len(recent_messages)} recent messages for analysis")

        if len(recent_messages) < 10:
            self.logger.info(f"[Reflection] Skipping reflection - only {len(recent_messages)} messages (need at least 10)")
            return
        ...
        try:
            ...
            self.logger.info(f"[Reflection] Successfully parsed {len(qa_pairs)} Q&A pairs from reflection response")
            ...
            self.logger.error(f"[Reflection] Error during reflection process: {e}")
    ...
    def get_recent_chat_history(self, limit: int = 20) -> List[Dict]:
        ...
        try:
            ...
            self.logger.debug(f"Error retrieving chat history from ChromaDB: {e}")
            return []
    ...
    def get_user_chat_history(self, user_id: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            self.logger.debug(f"Error retrieving user chat history from ChromaDB: {e}")
            return []
    ...
    def add_conversation_item(self, role: str, content: str, user_id: str = None,
        ...
        self.add_conversation_item_sync(role, content, user_id, metadata)
    ...
    def get_memory_context(self, user_id: str = None, current_topic: str = None) -> str:
        ...
        if current_topic:
            try:
                ...
                self.logger.debug(f"Error querying ChromaDB reflections: {e}")
    ...
    def get_memory_stats(self) -> Dict:
        ...
        try:
            ...
            self.logger.debug(f"Error getting ChromaDB counts: {e}")
